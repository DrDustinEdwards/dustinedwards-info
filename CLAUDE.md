...
5. Do not modify `.claude/settings.json` without explicit instruction. A PostToolUse hook rejects em dashes; that enforcement is deliberate. Note: a PostToolUse hook fires after the write lands and cannot block it. The blocking pattern is PreToolUse plus exit 2; see the foxhound repo for a tested implementation.
