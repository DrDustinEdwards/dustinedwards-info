// Ruling 115: ten header shot pairs, all 32 sheets as rewritten vs at 9b769c8.
// A copy of header-shots.mjs that swaps every tracked sheet from history-before/.
//
//   node scratchpad/history-shots.mjs
//
...
import { trackedSheets } from "./history-blocks.mjs";

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO = join(HERE, "..");
const OUT = join(HERE, "history-shots");
const PORT = 4273;
...
const putBefore = () => { for (const s of sheets) copyFileSync(join(HERE, "history-before", s.replaceAll("/", "__")), join(REPO, s)); };
