...
/**
 * Resolves production to its UUID, failing closed, then sweeps scratch databases an earlier run
 * leaked. Sweep first, because `finally` does not run on a hard kill. Only the prefix and only older
 * than the window, so a concurrent run is safe.
 */
function resolveAndSweep() {
...
/**
 * Exports every table production's migrations declare, in dependency order, then reads production's
 * integrity numbers, which every comparison after the restore is against.
 * @param {string} dir
 * @param {Record<string, number>} timings
 */
async function exportProd(dir, timings) {
  ...
  return { tables, dumps, live };
}

/**
 * Builds the scratch database from the migrations and loads the dump into it. `state.created` is set
 * the moment the database exists, so the caller's `finally` deletes it even when a later step throws.
 * @param {string} dir
 * @param {string[]} tables
 * @param {Array<{ name: string, inserts: number }>} dumps
 * @param {Record<string, number>} timings
 * @param {{ created: boolean }} state
 */
async function restoreScratch(dir, tables, dumps, timings, state) {
...
/**
 * The restored database against production: row counts, the rebuilt indexes and the schema.
 * @param {Record<string, unknown>} live
 */
async function verifyCounts(live) {
...
/**
 * Every MEDIA object against its MEDIA_BACKUP twin. Reads the buckets from outside the Worker, so the
 * mirror is asserted by something that is not maintaining it.
 */
async function verifyMediaMirror() {
...
async function main() {
  ...
  try {
    const { tables, dumps, live } = await exportProd(dir, timings);
    await restoreScratch(dir, tables, dumps, timings, state);
    await verifyCounts(live);
    await verifyMediaMirror();
  } finally {
    if (state.created) {
