 ...
 * @param {string} bName
 * @param {string[]} tables
 * @param {(label: string, condition: boolean, detail?: string) => void} check the calling test's collector
 */
function compareColumns(a, b, aName, bName, tables, check) {
  let comparisons = 0;
...
/**
 * schema.ts bundled and every migration applied to a fresh in-memory database, once for all three
...
async function loadSources() {
  ...
  for (const config of tableConfigs) {
    fromSchema.set(
      config.name,
      new Map(
        config.columns.map((/** @type {any} */ c) => [c.name, normalizeType(c.getSQLType())]),
      ),
    );
  }
  ...
  return { tableConfigs, fromSchema, fresh, migrations, migrationTables, virtual, fromMigrations };
}

/** @type {ReturnType<typeof loadSources> | undefined} */
let sources;
const loaded = () => (sources ??= loadSources());
...
test("schema: columns agree across schema.ts, the migrations and the database", { timeout: 180_000 }, async (t) => {
  ...
  try {
    const { fromSchema, migrations, migrationTables, virtual, fromMigrations } = await loaded();
 ...
 */
test("schema: indexes agree across schema.ts and the migrations", { timeout: 180_000 }, async (t) => {
  const { ok, fail, done } = collector();

  try {
    const { tableConfigs, fresh, migrationTables } = await loaded();
    ...
    fail("the index comparison could not run", String(error));
  }
...
test(
  "schema: the live database matches the migrations",
  {
    timeout: 180_000,
  ...
  async (t) => {
    ...
    try {
      const { migrationTables, fromMigrations } = await loaded();
      /*
      ...
      fail("the live comparison could not run", String(error));
    }
