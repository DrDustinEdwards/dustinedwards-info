 ...
 *
 * @see app/lib/search/ask-origin.mjs
 */

import test from "node:test";
import assert from "node:assert/strict";

import { ASK_ORIGIN_REFUSAL, askOriginVerdict } from "../app/lib/search/ask-origin.mjs";

const URL_HTTPS = "https://dustinedwards.dustin-edwards.workers.dev/search/ask";

test("SAME ORIGIN IS ACCEPTED", () => {
  const v = askOriginVerdict("https://dustinedwards.dustin-edwards.workers.dev", URL_HTTPS);
...
test("CROSS ORIGIN IS REFUSED, which is the whole point", () => {
  for (const origin of [
    ...
    const v = askOriginVerdict(origin, URL_HTTPS);
...
test("A SUFFIX ATTACK DOES NOT PASS", () => {
  ...
  assert.equal(
    askOriginVerdict("https://dustinedwards.dustin-edwards.workers.dev.evil.com", URL_HTTPS).ok,
    false,
  );
});
...
test("ABSENT Origin IS ACCEPTED, deliberately", () => {
  ...
  for (const absent of [null, undefined, ""]) {
    const v = askOriginVerdict(absent, URL_HTTPS);
...
test("THE STRING \"null\" IS REFUSED, and it is NOT the absent case", () => {
  ...
  const opaque = askOriginVerdict("null", URL_HTTPS);
  assert.equal(opaque.ok, false, "an opaque origin is not a missing origin");
  assert.equal(opaque.reason, "cross-origin");
  assert.equal(askOriginVerdict(null, URL_HTTPS).ok, true, "and JS null still passes");
});

test("SCHEME IS PART OF THE ORIGIN: http is not https", () => {
  assert.equal(
    askOriginVerdict("http://dustinedwards.dustin-edwards.workers.dev", URL_HTTPS).ok,
    false,
  );
});

test("PORT IS PART OF THE ORIGIN, so a dev server is not the site", () => {
  assert.equal(askOriginVerdict("http://localhost:5173", "http://localhost:5173/search/ask").ok, true);
  assert.equal(askOriginVerdict("http://localhost:5174", "http://localhost:5173/search/ask").ok, false);
});
...
test("IT COMPARES AGAINST THE REQUEST'S OWN ORIGIN, not a pinned constant", () => {
  ...
  assert.equal(askOriginVerdict("https://dustinedwards.info", apex).ok, true);
  assert.equal(
    askOriginVerdict("https://dustinedwards.dustin-edwards.workers.dev", apex).ok,
    false,
...
test("AN UNPARSEABLE REQUEST URL FAILS CLOSED", () => {
  const v = askOriginVerdict("https://evil.com", "not a url");
