...
Three small mechanics from the same layer, each boring and each necessary. Token comparison is constant-time with both operands hashed to a fixed width first, so length does not leak through the loop bound. Rate limiting reuses the site's existing synchronous-storage Durable Object rather than introducing a second mechanism. And a measurement warning that this project managed to document and then repeat anyway: a sequential test loop is not a burst, and thirty-six polite awaited requests against a limit of thirty per minute produced zero refusals from a limiter that was working correctly, while forty-five concurrent requests produced nineteen. Attack guards with real concurrency or the test measures nothing.

## The incident: the policy held and the inventory did not
...
This post was drafted by the agent and staged through the write path it describes, as a draft, with the operator marker on its commit. Its first publication, like every first publication on this site, required the human. The final article in the series covers the protocol layer built on top of this access: an MCP server designed to contain no policy at all.
