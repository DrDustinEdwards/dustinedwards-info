...
The property the method buys, stated once: the prose passes the same gates as the code, the database serves without holding custody, and the build can demonstrate, on every run, that what readers receive is what the repository says. This is the third post in [the series](/blog/ten-years-on-cloudflare), following [the palette method](/blog/a-color-palette-that-can-prove-itself); the next one covers [the reading experience built on this foundation under a no-client-JavaScript constraint](/blog/bells-and-whistles-zero-js).

## Update, August 2026

Step 4's asymmetry got its missing half. The original design fails the save whole when the commit cannot land, which is right, but it left the opposite window unhandled: commit landed, database write failed, repository and serving layer disagreeing until someone noticed. The save now retries the database write once; if the retry also fails, the divergence is recorded where the admin's sync status reads it and the error names the post, the commit that landed, and the repair. The commit is never reverted to make the database happy, for the reason this article already stated: the repository is the source of truth, so the serving layer converges toward it and never the reverse. Two related hardenings landed with it. A missing generated artifact now throws instead of quietly reading as an empty corpus, because on this pipeline the artifact is committed and gated, so its absence is a fault rather than a fresh clone. And the deploy script now refuses to ship any commit that continuous integration has not concluded green for, which closes the gap where a laptop could outrun the gates this article spends its middle third building.
