...
import { fileSha, sheetsSha, textSha } from "../scripts/lib/design-sheets.mjs";

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO = resolve(HERE, "..");
const OUT = join(HERE, "ds-styles.css");
const STAMP = join(HERE, ".cache/inputs-stamp.json");
...
const SHEETS = [
  ...
  // designing against a grid class it had never been sent. Keep this list in step with root.tsx
  // and the routes by hand: nothing checks it.
...
/**
 * `font-display: optional` becomes `swap`, IN THE SYNC ONLY.
 *
 * The site ships Inter as `optional` and preloads it, so the face is there before
 * first paint. A preview card or a design built from the bundle has no preload: the
 * face misses the ~100 ms window and `optional` keeps the fallback for the page's
 * whole life. Measured 2026-09-23 on the SiteLogoHeader card: the wordmark computed
 * Inter 700 and drew regular-weight Arial, because the `Inter Fallback` face is
 * local Arial declared for weights 100 to 900, which also suppresses synthetic
 * bold. The same card with `swap` drew Inter 700. The site's own `optional` is a
 * deliberate choice and is untouched.
 */
function swapFontDisplay(css) {
  let swapped = 0;
  const out = css.replace(/font-display\s*:\s*optional\b/g, () => {
    swapped++;
    return "font-display: swap";
  });
  return { css: out, swapped };
}
...
for (const rel of SHEETS) {
  ...
  const { css: swappedCss, swapped } = swapFontDisplay(noContrast);
  fontsSwapped += swapped;
  parts.push(`\n/* ===== ${rel} ===== */\n${rewriteUrls(stripImports(swappedCss, rel), rel)}`);
}

const out = parts.join("");
// No `optional` face may reach the bundle: it renders the fallback in every cold preview.
if (/font-display\s*:\s*optional\b/.test(out)) {
  throw new Error("font-display: optional survived into ds-styles.css; the sync's faces must swap");
}
...
console.error(`ds-styles.css: ${SHEETS.length} sheets, ${(out.length / 1024).toFixed(0)} KB, ${urls.length} url() target(s), ${fontsSwapped} optional face(s) set to swap`);
 ...
 * converter nor the driver ever spawns it. That is the hole `scripts/ds-resync.mjs`
 * closes by running this file itself; nothing catches a run that skipped it. Measured 2026-09-21: a driver run green on a ds-styles.css
...
/**
 * THE STAMP: what this run compiled, so a later run can prove the outputs are
 * still the outputs of the sheets on disk.
 *
 * ## WHY A STAMP AND NOT AN MTIME
 *
 * The defect being closed is a GREEN VERDICT ON A STALE ENTRY: the converter
 * copies ds-styles.css verbatim and reports nothing about its age, so a driver
 * run after a sheet changed compiled the previous flatten and every downstream
 * grade measured the wrong CSS. An mtime comparison is the obvious check and is
 * the wrong one here: this repo has already had a gate fail as stale on a clean
 * tree because a reverted file kept a new mtime. Content hashes have no such
 * failure mode, and a revert restores the hash exactly.
 *
 * ## WHAT IT BINDS, AND WHY EACH FIELD IS NEEDED
 *
 *   builder   this file's own source. A change to SHEETS, to the url() rewrite
 *             or to the comment strip changes the output, and the sheets it
 *             reads can all be untouched. Deliberately strict: any edit here
 *             asks for one regeneration.
 *   sheets    the list as compiled, so a sheet ADDED to SHEETS without a
 *             regeneration is named rather than inferred.
 *   sheetsSha the listed files' contents, which is the drift that actually
 *             happened.
 *   cssSha    the output itself, so a hand-edited or half-written ds-styles.css
 *             cannot pass by having fresh inputs.
 *
 * Gitignored with the outputs it describes: it is derived from them and a
 * committed copy would be a second owner, free to disagree.
 */
...
writeFileSync(
  ...
  JSON.stringify(
    {
      $comment: "GENERATED by build-inputs.mjs. Do not edit.",
      generatedAt: new Date().toISOString(),
      builder: fileSha(fileURLToPath(import.meta.url)),
      sheets: SHEETS,
      sheetsSha: sheetsSha(REPO, SHEETS),
      cssSha: textSha(out),
    },
    null,
    2,
...
console.error(`inputs-stamp.json: ${SHEETS.length} sheets bound`);
